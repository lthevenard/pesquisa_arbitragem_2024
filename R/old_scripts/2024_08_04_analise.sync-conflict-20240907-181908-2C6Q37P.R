...
df_citacoes_documentos %>% 
  ...
  mutate(categoria_de_tipo = fct_lump_min(categoria_de_tipo, 10, other_level = "Outros Tipos") %>% 
           fct_infreq() %>% 
           fct_rev() %>% 
           fct_relevel("Outros Tipos")) %>%
  ggplot(aes(y = autor_mais_citado %>% fct_infreq() %>% fct_rev(), fill = categoria_de_tipo)) +
  geom_bar() +
  labs(title = "Os 10 autores mais citados nos documentos das arbitragens",
       subtitle = subtitle,
       y = "Autor", x = "Número de Citações", fill = "Tipo de Documento: ") +
  scale_fill_manual(values = pal(6)[1:5] %>% rev(), guide = guide_legend(reverse = TRUE))

save_plot("26_autores_mais_citados_por_tipo.png")
