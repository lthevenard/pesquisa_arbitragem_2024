...
save_plot <- function(filename, path = "./2024_06_13/plots", height = 6, width = 10, dpi = 300, ...) {
  ggsave(filename=filename, height=height, width=width, path=path, dpi=dpi, ...)
}

## Sheet de arbitragens ----

frederico <- read_xlsx("2024_06_13/frederico.xlsx", sheet = "arbitragens", col_types = rep("text", 28)) %>%
...
julia <- read_xlsx("2024_06_13/julia.xlsx", sheet = "arbitragens", col_types = rep("text", 27)) %>%
...
soraya <- read_xlsx("2024_06_13/soraya.xlsx", sheet = "arbitragens", col_types = rep("text", 28)) %>%
...
daniel <- read_xlsx("2024_06_13/daniel.xlsx", sheet = "arbitragens", col_types = rep("text", 27)) %>%
  filter(!is.na(uf_arbitragem)) %>%
  mutate(pesquisador = "daniel",
         data_inicio = ifelse(str_detect(data_inicio, "\\d"), dmy(data_inicio) %>% as.character(), data_inicio))


gabriela <- read_xlsx("2024_06_13/gabriela.xlsx", sheet = "arbitragens", col_types = rep("text", 27)) %>%
...
pierre <- read_xlsx("2024_06_13/pierre.xlsx", sheet = "arbitragens", col_types = rep("text", 27)) %>%
  mutate(pesquisador = "pierre",
         data_inicio = ifelse(str_detect(data_inicio, "^\\d+$"), 
                              as_date(as.numeric(data_inicio), origin = "1899-12-30") %>% as.character(),
                              data_inicio),
         data_termo = ifelse(str_detect(data_termo, "^\\d+$"), 
...
df_arbitragens <- bind_rows(frederico, julia, soraya, daniel, gabriela, pierre)

divergencias <- read_xlsx("2024_06_13/divergencias_resolvidas.xlsx")

ids_divergencias_resolvidas <- divergencias$id_arbitragem %>% unique()

df_arbitragens <- df_arbitragens %>%
  filter(!(id_arbitragem %in% ids_divergencias_resolvidas))
...
casos_preenchimento <- tibble(
  caso = c("Mais de um preenchimento", "Apenas um preenchimento", "Nenhum preenchimento", "Preenchimento Anterior"),
  n = c(length(arbitragens_mais_de_um_preenchimento), length(arbitragens_apenas_um_preenchimento), length(arbitragens_nao_preenchidas) - 21, 21)
) 

casos_preenchimento %>%
  ggplot(aes(x = caso, y = n)) +
  geom_col(fill = c(rep("lightblue", 3), "gray")) +
...
write_csv2(divergencias, "2024_06_13/divergencias.csv")
...
write_csv2(casos_divergencias, "2024_06_13/casos_divergencias.csv")
...
blue_palette <- colorspace::choose_palette()
...
casos_divergencias %>%
  ...
  scale_fill_manual(values = blue_palette(5)[1:4]) +
  theme_bw() +
