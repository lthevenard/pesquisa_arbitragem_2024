...
subtitle <- "Pesquisa de arbitragens (2002-2023)"
...
ids_atores_institucionais <- df_atores %>%
  filter(idade == "N/A") %>% 
  .$id_ator

relacao_ator_arbitragem_clean %>%
  filter(tipo_relacao_consolidada == "Escritório de Advocacia" | (tipo_relacao_consolidada == "Advogado" & (id_ator %in% ids_atores_institucionais))) %>% 
  mutate(nome=str_remove(nome, "(Sociedade de)? A[dD]vogados|Advocacia") %>% str_squish()) %>% 
  count(nome, tipo_relacao, sort=T) %>% 
  group_by(nome) %>% 
  summarise(`Número de Arbitragens`=sum(n)) %>% 
